...
v5_overlap = zeros([i_square_size, i_square_size, i_dim3, v2_partition_numbers(1)-1, v2_partition_numbers(2)-1], 'uint8');
i_row_start = uint64(i_square_size/2+1);
for i_inc = 1:(v2_partition_numbers(1)-1)
    v2_rows = uint64([i_row_start:(i_row_start+i_square_size-1)]);

    i_col_start = uint64(i_square_size/2+1);
    for j_inc = 1:(v2_partition_numbers(2)-1)
        v2_cols = uint64([i_col_start:(i_col_start+i_square_size-1)]);
        
        
        %% Where the magic happens
        v5_overlap(:,:,:,i_inc,j_inc) = im_fl(v2_rows, v2_cols, :);
